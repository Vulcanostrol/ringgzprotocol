...
### MOVE;[x];[y];[move_type];&lt;color&gt;
  ...
  Here, X and Y represent the target coordinates of the piece that the player is placing (x horizontal/columns and y vertical/rows). Both of these coordinates range from 0-4. The move_type information block represents the type of piece the player is placing, these can be:
